  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-white">Viral Social Media Campaigns</h1>
          <p className="text-gray-400">Generate story-driven LinkedIn threads and aesthetic Instagram captions.</p>
        </div>
      </div>
        ...
        <div className="glass-card p-6 rounded-2xl">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input name="productName" required className="w-full bg-dark-900 border border-white/10 rounded-lg p-3 text-white focus:border-pink-500 outline-none transition-colors" placeholder="e.g. QuadriLabs SaaS" />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1">Target Audience</label>
              <input name="audience" required className="w-full bg-dark-900 border border-white/10 rounded-lg p-3 text-white focus:border-pink-500 outline-none transition-colors" placeholder="e.g. Agency Owners & B2B Founders" />
            </div>

            <button disabled={loading} type="submit" className="w-full py-3 bg-pink-600 hover:bg-pink-500 text-white font-semibold rounded-lg flex justify-center items-center gap-2 transition-colors disabled:opacity-50 mt-4">
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Share2 className="w-5 h-5" />}
              {loading ? "Generating Campaign..." : "Generate Social Campaign"}
            </button>
          </form>
        </div>
        ...
        <div className="glass-card p-6 rounded-2xl flex flex-col items-center justify-center min-h-[400px]">
          {loading ? (
            <div className="text-center">
              ...
              <p className="text-pink-400 font-medium animate-pulse">Writing viral hooks...</p>
            </div>
          ) : result ? (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full h-full flex flex-col">
              ...
              <button onClick={copyToClipboard} className="w-full py-3 bg-dark-800 hover:bg-dark-700 border border-white/10 text-white font-medium rounded-lg flex justify-center items-center gap-2 transition-colors relative overflow-hidden">
                ...
                {copied && <motion.div initial={{ opacity: 1 }} animate={{ opacity: 0 }} transition={{ duration: 1 }} className="absolute inset-0 bg-pink-500/20" />}
              </button>
            </motion.div>
          ) : (
            <div className="text-center text-gray-500">
              <Share2 className="w-12 h-12 mx-auto mb-3 opacity-20" />
              <p>Your viral social posts will appear here.</p>
            </div>
          )}
        </div>
      </div>
